...
export default function LoginForm() {
  ...
  const fetchCsrfToken = async () => {
    try {
      const response = await axios.get('https://vulkantechnologylabs.com:8000/token');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://vulkantechnologylabs.com:8000/api/uploadapplication', formData);
